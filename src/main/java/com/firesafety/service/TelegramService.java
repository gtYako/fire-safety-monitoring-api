...
public class TelegramService {
    ...
    // Лог отправок нужен, чтобы видеть: сообщение отправилось, пропущено или завершилось ошибкой.
    private final TelegramLogRepository telegramLogRepository;

    // Токен Telegram-бота берётся из application.yml или переменной окружения TELEGRAM_BOT_TOKEN.
    @Value("${telegram.bot-token:}")
    private String botToken;

    // Chat ID получателя берётся из application.yml или переменной окружения TELEGRAM_CHAT_ID.
    @Value("${telegram.chat-id:}")
    private String chatId;

    @Async
    public void sendAlert(String message) {
        // Если Telegram не настроен, приложение продолжает работать, но уведомление не отправляет.
        ...
        try {
            // Формируем HTTP-запрос к Telegram Bot API /sendMessage.
            ...
            // Ошибка отправки не должна ломать основной сценарий создания тревоги.
    ...
    private void saveTelegramLog(String message, TelegramLogStatus status, String error) {
        try {
            // Сохраняем результат попытки отправки Telegram-сообщения.
